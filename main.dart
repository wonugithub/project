...
class _KitchenPageState extends State<KitchenPage> {
  ...
  Future<void> fetchRecipes() async { if (_ingredients.isEmpty) { setState(() => _errorMessage = "최소 하나 이상의 식재료를 입력해주세요."); return; } setState(() { _isLoading = true; _errorMessage = ""; _recipes = []; }); try { final response = await http.post(Uri.parse('http://10.0.2.2:8000/api/recipes'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"ingredients": _ingredients})); if (response.statusCode == 200) { setState(() => _recipes = jsonDecode(utf8.decode(response.bodyBytes))); } else { setState(() => _errorMessage = "서버 오류가 발생했습니다."); } } catch (e) { setState(() => _errorMessage = "네트워크 통신에 실패했습니다."); } finally { setState(() => _isLoading = false); } }
